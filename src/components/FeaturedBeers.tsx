      ...
      <div className="max-w-5xl mx-auto px-4">
        <div className="flex items-end justify-between mb-10">
          <div>
            ...
            <h2 className="font-display text-2xl md:text-3xl">{t('Bieren')}</h2>
          </div>
        </div>
          ...
          {featured.map((beer, i) => (
            ...
            >
              <div className="bg-accent/8 border-b border-border/40 px-3 py-2.5 flex justify-between items-center">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-accent">{beer.style}</span>
                <span className="text-[10px] font-sans font-bold tabular-nums">{beer.abv}%</span>
              </div>
              <div className="p-3.5">
                <h3 className="font-display text-sm md:text-base mb-0.5 group-hover:text-accent transition-colors">{beer.name}</h3>
                {beer.breweryName && <p className="text-[10px] text-muted-foreground italic mb-2.5">{beer.breweryName}</p>}
                {beer.isHiddenGem && (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-success/10 text-success rounded-sm border border-success/20 text-[9px] font-bold uppercase tracking-wide">
                    <Star size={10} /> {t('Verborgen parel')}
                  </span>
                )}
              </div>
            </motion.div>
