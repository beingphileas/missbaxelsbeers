...
export default function Explore() {
  ...
  const handlePostClick = useCallback(
    (post: BlogPost) => {
      // Find related brewery or venue to pan to
      if (post.breweryId) {
        const brewery = breweries.find(b => b.id === post.breweryId);
        if (brewery) {
          setFocusLocation({ lat: brewery.lat, lng: brewery.lng });
          return;
        }
      }
      if (post.venueId) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="flex flex-col md:flex-row">
        {/* Left: Blog feed (60%) */}
        <div
          className={`md:w-[60%] md:h-[calc(100vh-56px)] md:overflow-y-auto ${
            isMobile && showMap ? 'hidden' : ''
          }`}
        >
          <div className="p-5 md:p-8 max-w-2xl mx-auto">
            <div className="mb-8">
              <p className="text-accent text-[10px] font-bold uppercase tracking-[0.3em] mb-2">
                EXPLORE
              </p>
              <h1 className="font-display text-3xl md:text-4xl mb-2">Baxel's Feed</h1>
              <p className="text-sm text-muted-foreground">
                Tastings, brouwerijverhalen en verborgen pareltjes.
              </p>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="text-muted-foreground text-sm">Nog geen verhalen gepubliceerd.</p>
              </div>
            ) : (
              <div className="space-y-6">
                {posts.map((post, i) => (
        ...
        {/* Right: Persistent map (40%) */}
        <div
          className={`md:w-[40%] md:sticky md:top-14 md:h-[calc(100vh-56px)] border-l border-border/60 ${
            isMobile && !showMap ? 'hidden' : 'h-[calc(100vh-100px)]'
          }`}
        >
          <MultiLayerMap
            breweries={breweries}
