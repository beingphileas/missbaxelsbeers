...
const BELGIUM_BOUNDS = {
  minLat: 49.4, maxLat: 51.6, minLng: 2.4, maxLng: 6.4,
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const mode = body.mode || "duplicates";
    const ids: string[] = body.ids || [];
    const batchSize = body.batch_size || 40;
    const offset = body.offset || 0;

    let query = supabase.from("venues").select("id, name, address, lat, lng").order("name");
    ...
    if (mode === "duplicates") {
      ...
      toProcess = toProcess.filter(v => dupCoords.has(`${v.lat.toFixed(6)},${v.lng.toFixed(6)}`));
    }

    if (mode === "suspect") {
      toProcess = toProcess.filter(v => v.address && v.address.trim().length > 5 && isSuspectLocation(v.lat, v.lng, v.address));
    }

    toProcess = toProcess.filter(v => v.address && v.address.trim().length > 5);
    ...
    for (const v of toProcess) {
      const coords = await geocodeBelgianAddress(v.address);
